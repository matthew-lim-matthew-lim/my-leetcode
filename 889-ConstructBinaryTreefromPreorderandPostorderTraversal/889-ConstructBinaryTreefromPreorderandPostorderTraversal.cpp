...
public:
    ...
    TreeNode* constructTree(int preStart, int preEnd, int postStart, vector<int>& preorder, vector<int>& postorder) {
        ...
        root->left = constructTree(preStart + 1, 
            preStart + numOfNodesInLeft, postStart, preorder, postorder);

        root->right = constructTree(preStart + numOfNodesInLeft + 1, 
            preEnd, postStart + numOfNodesInLeft, preorder, postorder);
